...
def main():
    ...
    try:
        new_list = [int(x[5:-4]) for x in os.listdir(f"frames/{vidfile}/")]
        new_list.sort()
    except:
        new_list = []    
    ...
    while(vidcap.isOpened()):
        ...
        if frame_exists: # and frame_no < 150
            if list_frames != [] and (frame_no+1) in list_frames:
                cv2.imwrite(f"frames/firstframes/get{vidfile}frame%d.jpg" % (frame_no+1), curr_frame)
                list_frames.remove((frame_no+1))
                if len(list_frames) == 0:
                    break
            
            if args.first_frame:
                cv2.imwrite(f"frames/firstframes/frame{vidfile}.jpg", curr_frame)
                break
            else:
                ...
                if frame_no < 0:
                    ...
                    if not os.path.exists('frames/'+vidfile):
                    ...
                    if np.std((av_crop_frame_o[2], av_crop_frame_c[2], av_crop_frame_n[2])) > 1.1: # Arbitrary value after experimentation. If something is failing, it could be this.
                        if av_crop_frame_o[2] < av_crop_frame_c[2] and av_crop_frame_n[2] < av_crop_frame_c[2]:
                            if args.write:
                                cv2.imwrite(f"frames/{vidfile}/frame%d.jpg" % (frame_no), ccrop_frame)
    ...
    np.savetxt('./info/info'+ vidfile +'.txt', frame_stats_list, fmt='%1.4f', delimiter=',', header="frame,min,max,range,std")
